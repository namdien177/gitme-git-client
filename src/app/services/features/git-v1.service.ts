...
export class GitService {
   ...
   * @param arrayPath
   */
  async getName(arrayPath: string[]): Promise<string> {
    ...
    if (isGitRepository) {
      return await this.getName(arrayPath.slice(0, arrayPath.length - 1));
    } else {
   ...
   */
  async getBranches(directory: string, oldBranches: RepositoryBranchSummary[] = []): Promise<RepositoryBranchSummary[]> {
  ...
  async fetch(repository: Repository, credentials: Account, branch: RepositoryBranchSummary) {
   ...
   * @param credentials
   */
  async checkoutBranch(repository: Repository, branchName: string, credentials?: Account) {
